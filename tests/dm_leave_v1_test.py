...
def test_dm_leave_input_error_valid_token_invalid_dm_id(register_three_users):
    ...
    dm_id = None
    response = requests.post(f"{url}/dm/leave/v1", json={'token': token, 'dm_id': dm_id})
...
def test_dm_leave_invalid_token_valid_dm_id(register_three_users):
    ...
    response = requests.post(f"{url}/dm/leave/v1", json={'token': invalid_token, 'dm_id': dm_id['dm_id']})
...
def test_dm_leave_access_error_valid_dm_id_user_not_member(register_three_users):
    ...
    member_id = register_three_users['id'][1]
    #run dm create with one user to another user
    response = requests.post(f"{url}/dm/create/v1", json={'token': member_token, 'u_ids': [member_id]})
    assert response.status_code == 200
    dm_id = response.json()
    dm_id = dm_id['dm_id']
    #run dm leave with wrong token and dm_id
    response = requests.post(f"{url}/dm/leave/v1", json={'token': nonmember_token, 'dm_id': dm_id})
    assert response.status_code == 403

def test_dm_leave_valid_input_creator_leaving(register_three_users):
    leaver_token = register_three_users['token'][0]
    member_id = register_three_users['id'][1]
    response = requests.post(f"{url}/dm/create/v1", json={'token': leaver_token, 'u_ids': [member_id]})
    assert response.status_code == 200
    dm_id = response.json()
    dm_id = dm_id['dm_id']
    response = requests.post(f"{url}/dm/leave/v1", json={'token': leaver_token, 'dm_id': dm_id})
    assert response.status_code == 200
    member_token = register_three_users['token'][1]
    response = requests.get(f"{url}/dm/details/v1?token={member_token}&dm_id={dm_id}")
    assert response.status_code == 200
