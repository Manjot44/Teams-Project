...
def test_listall_invalid_user():
    ...
    token = None
    response = requests.get(f"{url}/channels/listall/v2?token={token}")
...
def test_listall_valid_user_no_channel():
    ...
    response = requests.post(f"{url}/auth/register/v2", json={'email': 'email@email.com', 'password': 'password', 'name_first': 'first', 'name_last': 'last'})
