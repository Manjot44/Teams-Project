...
def test1obj():
    ...
    assert channels_list_v1(user_id)[0]["channel_id"] == 1
    assert channels_list_v1(user_id)[0]["name"] == 'channel1'
...
def test2obj():
    ...
    assert channels_list_v1(user_id)[0]["channel_id"] == 1
    assert channels_list_v1(user_id)[0]["name"] == 'channel1'
    assert channels_list_v1(user_id)[1]["channel_id"] == 2
    assert channels_list_v1(user_id)[1]["name"] == 'channel2'
    
#Testing to show only channels the user is part of show up
def testuserchannels():
    clear_v1()
    user_id1 = auth_register_v1("aBc123._%+-@aBc123.-.Co", "123456", "A", "A")
    user_id2 = auth_register_v1("abc@gmail.com", "thisIsPass13./", "Jerry", "Lin")
    ...
    assert channels_list_v1(user_id1)[0]["channel_id"] == 1
    assert channels_list_v1(user_id1)[0]["name"] == 'channel1'
    assert channels_list_v1(user_id1)[1]["channel_id"] == 3
    assert channels_list_v1(user_id1)[1]["name"] == 'channel3'
    assert channels_list_v1(user_id2)[0]["channel_id"] == 2
    assert channels_list_v1(user_id2)[0]["name"] == 'channel2'