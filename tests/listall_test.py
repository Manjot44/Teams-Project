...
def test_listall_invalid_user_id():
    ...
    with pytest.raises(error.AccessError):
        channels.channels_listall_v1(user_id)                     # tests listall function against invalid user_id
...
def test_listall_two_public_channel():
    other.clear_v1()
    user_id = auth.auth_register_v1('sanjamsingh@gmail.com', 'password', 'sanjam', 'singh')['auth_user_id']
    channel_id1 = channels.channels_create_v1(user_id, 'sanjam_channel1', True)['channel_id']
    channel_id2 = channels.channels_create_v1(user_id, 'sanjam_channel2', True)['channel_id']
