...
def test_email_missing_element(user_init):
    ...
    response = requests.post(f"{src.config.url}/auth/register/v2", json = user_init)
...
def test_email_repeat(user_init):
    requests.delete(f"{src.config.url}/clear/v1")
    requests.post(f"{src.config.url}/auth/register/v2", json = user_init) 
    response = requests.post(f"{src.config.url}/auth/register/v2", json = user_init)
    assert response.status_code == 400 
...
def test_handle_normal(user_init):
    ...
    response = requests.post(f"{src.config.url}/channels/create/v2", json = {"token": token, "name": "channel_name", "is_public": True})
    assert response.status_code == 200
    response_data = response.json()
    channel_id = response_data["channel_id"]

    response = requests.get(f"{src.config.url}/channel/details/v2?token={token}&channel_id={channel_id}")
    assert response.status_code == 200
    response_data = response.json()
