...
def test_invalid_message(register_three_users):
    ...
    response = requests.post(f"{url}/message/pin/v1", json = {"token": register_three_users["token"][0], "message_id": message_id + 1})
    assert response.status_code == 400
    requests.delete(f"{url}/clear/v1")

def test_double_pin(register_three_users):
    response = requests.post(f"{url}/channels/create/v2", json = {"token": register_three_users["token"][0], "name": "channel_name", "is_public": True})
    assert response.status_code == 200
    response_data = response.json()
    channel_id = response_data["channel_id"]
...
def test_not_owner(register_three_users):
    ...
    response_data = response.json()
    message_id = response_data["message_id"]
    response = requests.post(f"{url}/message/pin/v1", json = {"token": register_three_users["token"][1], "message_id": message_id})
